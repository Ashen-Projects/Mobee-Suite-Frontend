...
import { getDashboardOverview, type DashboardOverview } from "../../../redux/slices/dashboardRedux/dashboardApi";
...
export default function HomeDashboard() {
  ...
  const load = useCallback(async (background = false) => {
    ...
    try {
      const value = await getDashboardOverview({ fromDate, locationId: isAdmin ? locationId : "all", toDate });
  ...
  }, [fromDate, isAdmin, locationId, toDate]);
  ...
  const periodCaption = useMemo(() => fromDate === toDate ? "Selected day" : "Selected period", [fromDate, toDate]);
    ...
    <Stack spacing={{ xs: 1.75, sm: 2.25 }}>
      <Stack alignItems={{ xs: "stretch", md: "center" }} direction={{ xs: "column", md: "row" }} justifyContent="space-between" gap={1.5}>
        ...
        <Stack alignItems={{ xs: "stretch", sm: "center" }} direction={{ xs: "column", sm: "row" }} spacing={1}>
          ...
          {isAdmin ? <FormControl size="small" sx={{ minWidth: { xs: "100%", sm: 180 } }}>
      ...
      {noModuleAccess ? <Alert severity="info">Your account can open the dashboard, but no business modules are assigned. Ask an administrator to grant the required module permissions.</Alert> : null}
      ...
      {overview ? <>
        <Card sx={{ border: 1, borderColor: "divider" }}><CardContent sx={{ px: { xs: 1.5, sm: 2 }, py: 1.25, "&:last-child": { pb: 1.25 } }}>
          <Stack alignItems={{ xs: "flex-start", sm: "center" }} direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={0.75}>
            <Stack alignItems="center" direction="row" flexWrap="wrap" gap={0.75}><Chip color="primary" label={overview.meta.location.name} size="small" variant="outlined" /><Typography color="text.secondary" fontSize={11.5}>{periodCaption}: {overview.meta.fromDate} to {overview.meta.toDate}</Typography></Stack>
            <Typography color="text.secondary" fontSize={10.75}>Updated {formatDateTime(overview.meta.generatedAt)}</Typography>
          </Stack>
        </CardContent></Card>

        {overview.sales ? <Box sx={{ display: "grid", gap: 1.5, gridTemplateColumns: { xs: "1fr", sm: "repeat(2, minmax(0, 1fr))", xl: overview.visibility.financials ? "repeat(5, minmax(0, 1fr))" : "repeat(4, minmax(0, 1fr))" } }}>
          ...
          {overview.visibility.financials ? <DashboardMetricCard caption={`COGS ${fCurrency(overview.sales.costOfGoods ?? 0)}`} icon={<TrendingUpRoundedIcon fontSize="small" />} label="Gross profit" tone="success" value={fCurrency(overview.sales.grossProfit ?? 0)} /> : null}
        ...
        {overview.visibility.financials && overview.inventory ? <Box sx={{ display: "grid", gap: 1.5, gridTemplateColumns: { xs: "1fr", sm: "repeat(2, 1fr)", lg: "repeat(4, 1fr)" } }}>
          <DashboardMetricCard caption="Available stock only" icon={<Inventory2OutlinedIcon fontSize="small" />} label="Stock cost value" tone="info" value={fCurrency(overview.inventory.costValue ?? 0)} />
          <DashboardMetricCard caption="Potential retail value" icon={<SellOutlinedIcon fontSize="small" />} label="Stock MRP value" tone="success" value={fCurrency(overview.inventory.mrpValue ?? 0)} />
          {overview.purchasing ? <DashboardMetricCard caption="Selected period" icon={<LocalShippingOutlinedIcon fontSize="small" />} label="Purchase cost" tone="warning" value={fCurrency(overview.purchasing.costTotal ?? 0)} /> : null}
          {overview.purchasing ? <DashboardMetricCard caption="Selected period" icon={<AccountBalanceWalletOutlinedIcon fontSize="small" />} label="Supplier balance" tone="error" value={fCurrency(overview.purchasing.balanceAmount ?? 0)} /> : null}
        </Box> : null}
