...
import { Autocomplete, Badge, Box, Button, Card, Chip, Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControlLabel, IconButton, InputAdornment, MenuItem, Stack, Switch, Tab, Tabs, TextField, Tooltip, Typography, useMediaQuery, useTheme } from "@mui/material";
...
import { createSupplier, getSupplier, getSuppliers, reserveSupplierCode, updateSupplier, updateSupplierProductStatus, updateSupplierStatus, upsertSupplierProduct, type Supplier, type SupplierDetail, type SupplierInput, type SupplierPaymentMethod, type SupplierProduct } from "../../../redux/slices/purchaseRedux/supplierRedux";
import { USER_PERMISSIONS } from "../../../utils/constants";
import { fCurrency } from "../../../utils/formatNumber";

const PAGE_SIZE = 10;
const PAGE_SIZE_OPTIONS = [10, 15, 25, 50];
const emptySupplier: SupplierInput = { address: null, code: "", contactPerson: null, creditLimit: 0, email: null, isActive: true, name: "", paymentTermDays: 0, phone: null, preferredPaymentMethod: "bankTransfer" };
const paymentMethodLabels: Record<SupplierPaymentMethod, string> = { bankTransfer: "Bank transfer", card: "Card", cash: "Cash", cheque: "Cheque" };
...
export default function Suppliers() {
  ...
  const openEdit = async (row: Supplier, initialTab = 0) => {
    try {
      ...
      setEditing(detail);
      setForm({ address: detail.address, code: detail.code, contactPerson: detail.contactPerson, creditLimit: Number(detail.creditLimit), email: detail.email, isActive: detail.isActive, name: detail.name, paymentTermDays: detail.paymentTermDays, phone: detail.phone, preferredPaymentMethod: detail.preferredPaymentMethod });
  ...
  const saveSupplier = async () => {
    ...
    try {
      ...
      const result = editing ? await updateSupplier(editing.id, { address: payload.address, code: payload.code, contactPerson: payload.contactPerson, creditLimit: payload.creditLimit, email: payload.email, name: payload.name, paymentTermDays: payload.paymentTermDays, phone: payload.phone, preferredPaymentMethod: payload.preferredPaymentMethod }) : await createSupplier(payload);
  ...
  const columns = useMemo<GridColDef<Supplier>[]>(() => [
    ...
    { field: "preferredPaymentMethod", headerName: "Preferred Payment", minWidth: 170, valueFormatter: (value) => paymentMethodLabels[value as SupplierPaymentMethod] ?? "—" },
  ...
  return <>
    ...
    <Stack spacing={{ xs: 2, sm: 2.5 }}>
      <Stack alignItems={{ xs: "stretch", sm: "center" }} direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={2}>
        <Stack alignItems="center" direction="row" spacing={1}><BusinessOutlinedIcon color="primary" sx={{ fontSize: 22 }} /><Typography variant="h4">Suppliers</Typography></Stack>
    ...
    <Dialog fullScreen={mobile} fullWidth maxWidth="lg" onClose={() => setEditorOpen(false)} open={editorOpen} slotProps={{ paper: { sx: { borderRadius: { sm: 3 }, height: { sm: "88dvh" } } } }}>
      <DialogTitle component="div"><Stack alignItems="center" direction="row" justifyContent="space-between"><Box><Typography variant="h5">{editing ? editing.name : "Create Supplier"}</Typography><Typography color="text.secondary" variant="body2">Supplier information and product agreements.</Typography></Box><Stack alignItems="center" direction="row" gap={1}>{editing && can(USER_PERMISSIONS.SUPPLIERS_UPDATE) ? <FormControlLabel control={<Switch checked={form.isActive} onChange={() => setStatusTarget(editing)} />} label="Active" /> : null}<Button disabled={saving} onClick={() => void saveSupplier()} variant="contained">{editing ? "Save" : "Create"}</Button></Stack></Stack></DialogTitle>
