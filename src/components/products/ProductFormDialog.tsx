  ...
  >
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      ...
      <Card sx={panelSx}><Stack spacing={2.5}>
        ...
        {kind !== "variable" ? <><Box sx={{ display: "grid", gap: 2, gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))" } }}><TextField disabled helperText="Set this when adding approved stock." inputProps={{ inputMode: "decimal", pattern: "[0-9]*[.]?[0-9]{0,2}" }} label="MRP Price" onChange={(event) => setMrpPrice(event.target.value)} type="text" value={mrpPrice} /><TextField disabled helperText="Set this when adding approved stock." inputProps={{ inputMode: "decimal", pattern: "[0-9]*[.]?[0-9]{0,2}" }} label="Lowest Selling Price" onChange={(event) => setLowestSellingPrice(event.target.value)} type="text" value={lowestSellingPrice} /></Box>
        <TextField helperText="Optional catalog code" label="Product Code / SKU" onChange={(event) => setSku(event.target.value.toUpperCase())} value={sku} /></> : <Box sx={{ bgcolor: "action.hover", border: 1, borderColor: "divider", borderRadius: 2, p: 2 }}><Typography fontWeight={700} variant="body2">Prices are managed by child products</Typography><Typography color="text.secondary" mt={0.5} variant="caption">This parent groups its variations and does not have an independent selling price or MRP.</Typography></Box>}
